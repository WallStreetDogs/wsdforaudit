...
contract NFTSale is Context, Ownable, ReentrancyGuard {
    ...
    constructor (address wsdgToken, address nftToken, address _feeAddress, address _authAddress, uint256, uint256 _nftWSDGPrice) {
        WSDG_TOKEN = IERC20(wsdgToken);
        NFT_TOKEN = IERC1155(nftToken);
    
        feeAddress = _feeAddress;
        authAddress = _authAddress;

        nftWSDGPrice = _nftWSDGPrice;
    }

    event Buy(uint256 indexed buyId, address account);

    uint256 public nftWSDGPrice;
    address public feeAddress;
    address public authAddress;
    IERC20 private WSDG_TOKEN;
    ...
    function buyNFT(uint256 amount) public nonReentrant {
        ...
        uint256 costWSDG = nftWSDGPrice.mul(amount).mul(1e18);
        WSDG_TOKEN.transferFrom(_msgSender(), feeAddress, costWSDG);
    }
    ...
    function setWSDGPrice(uint256 price) public onlyOwner nonReentrant {
      nftWSDGPrice = price;
    }
